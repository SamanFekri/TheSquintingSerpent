...
class SnakeEnv:
    ...
    Observation returned by get_obs():
      (grid, hunger, smell)
        grid: np.float32 (1, 2N+1, 2N+1) -> encoded window around the head
              ...
              [-0.99 … -0.01] = snake body (head most negative, tail closest to 0)
               1.0 = food
               0.0 = empty
    ...
    def get_obs(self):
        ...
        hx, hy = self.snake[0]
        # Gradient encode the body so the head is most negative and the tail
        # is closest to zero. Values are linearly spaced from tail to head.
        if len(self.snake) == 1:
            snake_values = {self.snake[0]: -0.99}
        else:
            gradient = np.linspace(-0.01, -0.99, len(self.snake), dtype=np.float32)
            snake_values = {pos: float(val) for pos, val in zip(reversed(self.snake), gradient)}
        ...
        for j in range(size):
            for i in range(size):
                ...
                if self.walls[y, x] == 1:
                    grid_2d[j, i] = -1.0
                elif (x, y) in snake_values:
                    grid_2d[j, i] = snake_values[(x, y)]
